...
При прослушивании всех интерфейсов (`0.0.0.0`) строка состояния показывает
реальный LAN IPv4, который нужно указывать на другом компьютере. Сам сервер при
этом остаётся доступен на всех сетевых адаптерах.
